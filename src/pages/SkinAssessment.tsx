import { useEffect, useState } from "react"; import { SidebarProvider, SidebarInset } from "@/components/ui/sidebar"; import { AppSidebar } from "@/components/ui/app-sidebar"; import { AppHeader } from "@/components/ui/app-header"; import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"; import { Textarea } from "@/components/ui/textarea"; import { Button } from "@/components/ui/button"; import { Paintbrush, SaveIcon } from "lucide-react";

interface Annotation { areaId: string; notes: string; }

export default function SkinAssessmentForm() { const [annotations, setAnnotations] = useState<Annotation[]>([]); const [selectedArea, setSelectedArea] = useState<string | null>(null); const [notes, setNotes] = useState<string>("");

useEffect(() => { const saved = localStorage.getItem("skinAssessment"); if (saved) { setAnnotations(JSON.parse(saved)); } }, []);

-- FINAL SECURITY FIX: Address remaining vulnerable tables
...
-- Create additional role-based policies for medical staff access
...
USING (
  ...
  (has_role(auth.uid(), 'registered_nurse'::app_role) OR has_role(auth.uid(), 'doctor'::app_role) OR has_role(auth.uid(), 'caregiver'::app_role))
);
